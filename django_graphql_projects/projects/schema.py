...
class ProjectInput(graphene.InputObjectType):
    ...
    developer = graphene.List(DeveloperInput)
    year = graphene.Int()

# create mutation for developers
class CreateDevelooper(graphene.Mutation):
    class Arguments:
    ...
    def mutate(root, info, input=None):
        ...
        return CreateDevelooper(ok=ok, developer=developer_instance)
...
class CreateProject(graphene.Mutation):
    ...
    def mutate(root, info, input=None):
        ...
        for developer_input in input.developers:
            developer = Developer.objects.ger(pk=developer_input.id)
    ...
    def mutate(root, info, input=None):
        ...
        if project_instance:
            ...
            for developer_input in input.developers:
                ...
                if developer is None:
                    return UpdateDeveloper(ok=False, project=project_instance)
                developers.append(developer)
            project_instance.title = input.title
            project_instance.year = input.year.save()
            project_instance.developers.set(developers)
...
class Mutation(graphene.ObjectType):
    create_developer = CreateDevelooper.Field()
